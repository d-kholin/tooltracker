            ...
            <div className="card h-100">
              ...
              <div className="card-body">
                ...
                <div className="d-flex gap-2">
                  <a href={`/edit/${tool.id}`} className="btn btn-sm btn-secondary">Edit</a>
                  {tool.borrower ? (
                    <form method="post" action={`/return/${tool.id}`} className="m-0">
                      <button type="submit" className="btn btn-sm btn-warning">Return</button>
                    </form>
                  ) : (
                    <a href={`/lend/${tool.id}`} className="btn btn-sm btn-primary">Lend</a>
                  )}
                </div>
              </div>
            </div>
